...
async function create () {
  ...
  const tasks = [
    ...
    },
    {
      ...
      task: async () => {
        if (!cmdExists('hub')) {
          throw new Error('Hub is required. Installation instructions: https://hub.github.com/')
        }

        await execa('git config --global hub.protocol https', { shell: true })
        await execa(`git init`, { cwd, shell: true })
        await execa(`hub create -p -d "${options.description}" luckyorange/${options.name}`, { cwd, shell: true })
        await execa(`git remote set-url origin https://github.com/luckyorange/${options.name}.git`, { cwd, shell: true })
      }
    },
    {
