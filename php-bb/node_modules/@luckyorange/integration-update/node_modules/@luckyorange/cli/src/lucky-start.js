...
const pkgName = pkg.name.replace('@luckyorange/', '')
const mode = program.mode

// Get start command
const parsed = program.parse(process.argv)
let startCmdArray = process.argv.slice(2)
let finalCommandArray = [...startCmdArray]
...
const tasks = new Listr([
  {
    ...
    task: () => new Listr(config.start.services.map(service => {
      ...
      if (service.namespace) kubectlOptions += ` --namespace=${service.namespace}`
        ...
        task: (ctx, task) => {
          return new Promise(async (resolve, reject) => {
            try {
              ...
              await execa('gcloud', gcloudOptions.split(' '))
  ...
  },
  {
    ...
    skip: async () => {
      const proxyPort = await getPort({ port: 8002, host: 'localhost' })
    ...
    task: async (ctx, task) => {
      return new Promise(async (resolve, reject) => {
        ...
        try {
          const { stdout } = await execa('kubectl', 'get pod dev-proxy'.split(' '))
        } catch (error) {
          if (error.message.includes('NotFound')) {
            task.output = 'Creating proxy deployment: dev-proxy'
            await execa('kubectl', `run dev-proxy --image=hgascoigne/anyproxy:latest --wait=true --command -- anyproxy --intercept`.split(' '))
          }
        }
  ...
  },
  {
    ...
    task: async (ctx, task) => {
      ...
      if (program.project === 'lucky-orange' && process.env.GCLOUD_SERVICE_ACCOUNT_PROD) {
        fs.writeFileSync('./gcloud-service-account.json', process.env.GCLOUD_SERVICE_ACCOUNT_PROD)
      } else if (program.project === 'lucky-orange-staging' && process.env.GCLOUD_SERVICE_ACCOUNT_STAGING) {
      ...
      const [secrets] = await client.listSecrets({
        parent: `projects/${program.project}`
      })
      ...
      responses.forEach(([version], index) => {
    ...
    task: async (ctx, task) => {
      ...
      if (mode === 'dev') {
        ...
        if (startCmd.includes('micro-dev')) {
          ...
          const microPort = await getPort({ host: '127.0.0.1', port: 3000 })
